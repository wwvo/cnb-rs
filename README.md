...
- 如果你希望第一跳和后续附件都走 GitHub，可以改用 GitHub 托管的脚本入口，并把下载源传给安装脚本：
...
- 在 shell 管道里，如果你用环境变量切换下载源，请把 `CNB_RS_INSTALL_SOURCE=github` 放在 `bash` 这一侧；写在 `curl` 前面不会传递给安装脚本
...
**文案约定：** 面向用户的**错误、提示与自定义帮助**（如顶层 `错误:`、补全子命令说明）统一为**简体中文**。说明命令行时采用**方案 B**：对外统称「**参数**」；其中形如 `--foo`、`-h` 的称为「**选项**」，命令行上位置传入的称为「**位置参数**」（具体以各子命令 `--help` 为准）。依赖库自带的英文（如部分 clap 解析错误）若需完全中文化，需在后续迭代中逐步包装或替换。
